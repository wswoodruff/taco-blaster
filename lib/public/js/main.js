...
const initSocket = async () => {
    ...
    const wsLocation = window.location.host.includes('3001') ? 'localhost:3000' : window.location.host;

    if (wsLocation.includes('https')) {
        RoomClient = new Nes.Client(`wss://${wsLocation}`);
    }
    else {
        RoomClient = new Nes.Client(`ws://${wsLocation}`);
    }
